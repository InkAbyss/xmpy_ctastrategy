...
from xmpy.包_交易核心.模块_工具 import 加载json文件, 保存json文件, 提取合约代码, 四舍五入到指定值, 合约_交易所转英文
...
class 类_CTA引擎(基础引擎):
    ...
    def 处理行情事件(self, 事件: 类_事件) -> None:
        ...
        # 获取关联策略列表
        策略列表: list = self.合约策略映射[行情实例.合约_交易所]
        if not 策略列表:
            return
    ...
    def 检查停止单(self, 行情实例: 类_行情数据) -> None:
        ...
        for 停止单实例 in list(self.所有停止单.values()):
            ...
            if 多单触发 or 空单触发:
                ...
                合约信息: 类_合约数据 | None = self.主引擎.获取合约详情(停止单实例.合约_交易所)
    ...
    # ----------------- 新增 -------------------------
    def 获取账户详情(self, 账户标识) -> int | None:
        # 账户详情 = self.主引擎.获取账户详情('CTP.200571')
        账户详情: 类_账户数据 = self.主引擎.获取账户详情(账户标识)
        if 账户详情:
            return 账户详情
        else:
            return None
    
    def 获取所有账户(self) -> int | None:
        所有账户详情: 类_账户数据 = self.主引擎.获取所有账户()
        if 所有账户详情:
            return 所有账户详情
        else:
            return None

    def 获取持仓详情(self, 持仓标识) -> int | None:
        持仓详情: 类_持仓数据 = self.主引擎.获取持仓详情(持仓标识)
        if 持仓详情:
            return 持仓详情
        else:
            return None

    def 获取所有持仓(self) -> int | None:
        所有持仓详情: 类_持仓数据 = self.主引擎.获取所有持仓()
        if 所有持仓详情:
            return 所有持仓详情
        else:
            return None
    ...
    def 记录日志(self, 日志内容: str, 策略实例: 类_CTA策略模板 | None = None, 日志级别: int = 20) -> None:
        """
        创建cta引擎日志事件
        """
        if 策略实例:
        ...
        日志实例: 类_日志数据 = 类_日志数据(消息内容=日志内容, 网关名称=应用名称, 日志级别 = 日志级别)
